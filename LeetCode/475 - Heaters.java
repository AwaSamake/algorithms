 ...
 *    not exceed 25000.
 *    Positions of houses and heaters you are given are non-negative and will 
 *    not exceed 10^9.
...
public class Solution {
	public int findRadius(int[] houses, int[] heaters) {
		...
		for (int house: houses) {
			...
			int distToPre = index - 1 >= 0 ? house - heaters[index - 1] : Integer.MAX_VALUE;
			int disToNext = index < heaters.length ? heaters[index] - house : Integer.MAX_VALUE;
			
			radius = Math.max(radius, Math.min(disToNext, distToPre));
		}
		return radius;
	}
}