...
public class Solution {
	public String removeKdigits(String num, int k) {
		char[] newNum = new char[num.length() - k];
		int index = 0;
		for (int i = 0; i < num.length() && index < newNum.length; ++i) {
			while (index > 0 && newNum[index - 1] > num.charAt(i) && k > 0) {
				k--;
				index--;
			}
			newNum[index++] = num.charAt(i);
		}
		
		int zeros = 0;
		while (zeros < newNum.length && newNum[zeros] == '0') {
			zeros++;
		}
		return zeros == newNum.length ? "0" : new String(newNum, zeros, newNum.length - zeros);
	}
}