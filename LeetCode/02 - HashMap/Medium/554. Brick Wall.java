 ...
 *
 * 554. Brick Wall
 *    There is a brick wall in front of you. The wall is rectangular and has several rows 
 *    of bricks. The bricks have the same height but different width. You want to draw a 
...
public class Solution {
	public int leastBricks(List<List<Integer>> wall) {
		...
		for (Map.Entry<Integer, Integer> entry: edgeCount.entrySet()) {
			...
			if (location != 0 && location != position && count > max) {
				max = count;
			}
		}
