 ...
 *    makes a guess, you provide a hint that indicates how many digits in said guess 
 *    match your secret number exactly in both digit and position (called "bulls") and 
 *    how many digits match the secret number but locate in the wrong position (called 
 *    "cows"). Your friend will use successive guesses and hints to eventually derive the 
 *    secret number.
 *    
 *    For example:
 *    
...
public class Solution {
	public String getHint(String secret, String guess) {
		...
		for (int i = 0; i < secret.length(); ++i) {
			...
			int count = charCount.getOrDefault(g, 0);
			if (g != secret.charAt(i) && count > 0) {
				charCount.put(g, count - 1);
				cows++;
			}
		}
