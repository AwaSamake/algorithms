...
public class Solution {
	public int findPairs(int[] nums, int k) {
		...
		HashSet<Integer> pairedSmall = new HashSet<Integer>();
		HashSet<Integer> pairedLarge = new HashSet<Integer>();
		
		for (int num : nums) {
			if (!pairedLarge.contains(num) && set.contains(num + k)) {
				count++;
				pairedLarge.add(num);
				pairedSmall.add(num + k);
			}
			if (!pairedSmall.contains(num) && k != 0 && set.contains(num - k)) {
				count++;
				pairedSmall.add(num);
				pairedLarge.add(num - k);
			}
			set.add(num);
		}
		return count;
